...
//-----{调用ALU模块}begin
    reg   [15:0] alu_control;  // ALU控制信号
    reg   [31:0] alu_src1;     // ALU输入数1
    reg   [31:0] alu_src2;     // ALU输入数2
...
//根据实际需要输入的数修改此小节，
//触摸屏上每有一次输入，都会产生一次
    //当input_sel为00时，表示输入数据为控制信号，即alu_control
    always @(posedge clk)
    begin
        if (!resetn)
        begin
    ...
    begin
        case(display_number)
            ...
            begin
                display_valid <= 1'b1;
                display_name  <= "CONTR";
                display_value <={16'd0, alu_control};
            end
            6'd4 :
            begin
                display_valid <= 1'b1;
